...
import pt.tecnico.dsi.openstack.common.models.{UnexpectedStatus, Usage}
...
class QuotasSpec extends Utils:
  ...
  "Quotas service" should:
    "get quotas for a project (existing id)" in withStubProject.use { project =>
    ...
    "get usage quotas for a project (existing id)" in withStubProject.use { project =>
    ...
    "get default quotas for a project (existing id)" in withStubProject.use { project =>
    ...
    s"show quotas" in withStubProject.use { project =>
    ...
    s"show quota usage" in withStubProject.use { project =>
      quotas.getUsage(project.id).map { quotaUsage =>
        //This line is a fail fast mechanism, and prevents false positives from the linter
        println(show"$quotaUsage")
        """show"$quotaUsage"""" should compile
      }
    }
